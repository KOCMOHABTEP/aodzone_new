      ...
      <div className={styles.grid}>
        <div className={clsx(styles.gridColumn, styles.gridColumnLeft)}>
          {firstColumnItems.map((props) => (
            <Link href="news/1" className={styles.gridItem} key={props.id}>
              <NewsItem {...props} />
            </Link>
          ))}
        </div>
        <div className={clsx(styles.gridColumn, styles.gridColumnRight)}>
      ...
      <div className={styles.head}>
        ...
        {isAuth && (
          <Link href="news/create">
            <Button text="Предложить новость" buttonClassName={styles.btn} />
          </Link>
        )}
      </div>
      {renderItemsGrid()}
